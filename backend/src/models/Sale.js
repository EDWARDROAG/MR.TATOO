 ...
 * ============================================================
 * PROPÓSITO:
 *   Modelo / acceso a datos — Sale.js.
 *
 * FUNCIONES / API (contrato exporta):
 *   create, findById, findAll, updateReceipt, cancel, restoreProductStock,
 *   getSalesByDateRange, getSummaryByDate, getSummaryByDateRange,
 *   getSummaryBySeller, getLastInvoiceNumber
 *
 * DEPENDENCIAS CLAVE:
 *   database, Product
 *
...
const create = async (saleData, client = null) => {
...
const cancel = async (id, motivo, client = null) => {
    const exec = client ? client.query.bind(client) : query;
    const result = await exec(
        `UPDATE sales 
...
const restoreProductStock = async (producto_id, client = null) => {
    const exec = client ? client.query.bind(client) : query;
    const result = await exec(
        `UPDATE products 
