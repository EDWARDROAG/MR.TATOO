 ...
 * ============================================================
 * PROPÓSITO:
 *   Controlador HTTP — endpoints de sale.
 *
 * FUNCIONES / API (contrato exporta):
 *   createSale, getSales, getSaleById, cancelSale, uploadTransferReceipt,
 *   generateSaleInvoice, getSalesSummaryBySeller
 *
 * DEPENDENCIAS CLAVE:
 *   Sale, SaleItem, Product, Log, invoiceGenerator, imageOptimizer, database
 *
...
const createSale = async (req, res) => {
    try {
        ...
        if (!metodo_pago || !['efectivo', 'transferencia', 'tarjeta'].includes(metodo_pago)) {
            return res.status(400).json({
                success: false,
        ...
        // Comprobante fuera de la TX (I/O); venta+ítems+stock atómicos
        ...
        const sale = await transaction(async (client) => {
            ...
            for (const item of itemsValidados) {
                ...
                await Product.markAsSold(item.producto_id, client);
            }
    ...
    try {
        ...
        const sale = await transaction(async (client) => {
            for (const item of existingSale.items) {
                await Sale.restoreProductStock(item.producto_id, client);
            }
            return Sale.cancel(parseInt(id), motivo || 'Cancelada por administrador', client);
        });
